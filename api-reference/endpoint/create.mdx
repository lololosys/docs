...
6. **Use the WordPress plugin** for enhanced SEO features

### Job Tracking

After creating a post, use the returned `jobId` to track progress:
...
See the [Job Status endpoint](/api-reference/endpoint/get) for complete tracking details.

<Note>
  For a complete list of all available parameters including advanced settings,
  see our [Parameters Reference](/essentials/parameters).
</Note>
